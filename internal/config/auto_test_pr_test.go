...
//	c. malformed JSON → returns typed error (not a panic).
...
func TestLoadRigSettings_AutoTestPRDisabledBlockShipsCleanly(t *testing.T) {
	...
	// validate, so the per-rig settings JSON can carry the stanza ahead
	// of the operator-driven `gt auto-test-pr enable` flip.
	body := `{
	  "type": "rig-settings",
	  "version": 1,
