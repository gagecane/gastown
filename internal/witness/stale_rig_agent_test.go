...
func TestDetectStaleRigAgentHeartbeats_DisabledByZero(t *testing.T) {
	...
	res := DetectStaleRigAgentHeartbeats(t.TempDir(), "testrig", nil, 0, "", 0, 0)
	if res == nil {
...
func TestDetectStaleRigAgentHeartbeats_NoSessionNoEscalation(t *testing.T) {
	...
	townRoot := t.TempDir()
	res := DetectStaleRigAgentHeartbeats(townRoot, "testrig", nil, time.Hour, "", 0, 0)
	if res.Checked != 2 {
...
func TestDetectStaleRigAgentHeartbeats_SelfSkip(t *testing.T) {
	...
	res := DetectStaleRigAgentHeartbeats(townRoot, rigName, nil, time.Hour, witnessSession, 0, 0)
	if res.Checked != 2 {
		t.Fatalf("Checked = %d, want 2", res.Checked)
	}
