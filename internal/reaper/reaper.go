...
func hasWorkingSetChanges(ctx context.Context, runner sqlRunner) bool {
	var count int
	if err := runner.QueryRowContext(ctx, "SELECT COUNT(*) FROM dolt_status").Scan(&count); err != nil {
		return true
	}
	return count > 0
}
...
func Scan(db *sql.DB, dbName string, maxAge, purgeAge, mailDeleteAge, staleIssueAge time.Duration) (*ScanResult, error) {
	...
	reapQuery := fmt.Sprintf(
		"SELECT COUNT(*) FROM wisps w %s %s %s WHERE %s AND w.created_at < ? AND w.issue_type != 'agent' AND %s AND %s AND closed_molecule_step.issue_id IS NULL",
		parentJoin, trackedJoin, moleculeStepExcludeJoin, openWispStatusWhere, parentWhere, trackedWhere)
	...
	// parents) / depends_on_issue_id (issue parents); the legacy single-target
...
func Reap(db *sql.DB, dbName string, maxAge time.Duration, dryRun bool) (*ReapResult, error) {
	...
	whereClause := fmt.Sprintf(
		"%s AND w.created_at < ? AND w.issue_type != 'agent' AND %s AND %s AND closed_molecule_step.issue_id IS NULL", openWispStatusWhere, parentWhere, trackedWhere)
	...
	if dryRun {
		...
		countQuery := fmt.Sprintf("SELECT COUNT(*) FROM wisps w %s %s %s WHERE %s", parentJoin, trackedJoin, moleculeStepExcludeJoin, whereClause)
	...
	idQuery := fmt.Sprintf(
		...
		parentJoin, trackedJoin, moleculeStepExcludeJoin, whereClause, DefaultBatchSize)
	...
	if totalClosed > 0 {
		...
		// commit" warning in dolt.log (gu-leuwr). Runs on the pinned conn so the
		// guard observes the same session that holds the just-committed working set.
		if hasWorkingSetChanges(ctx, conn) {
...
// autoCloseWhereClause builds the WHERE predicate (sans leading WHERE) that
// selects stale auto-close candidates for dbName.
//
// IMPORTANT: staleInfraRoleExcludeSQL("i") returns a fragment containing literal
// SQL LIKE wildcards (e.g. `LIKE '%role_type: refinery%'`). It must be passed as
// a %s *argument* to fmt.Sprintf, never concatenated into the format string —
// otherwise fmt re-scans the '%r' in '%role_type' as a verb, consumes dbName as
// its operand, and corrupts the SQL to `%!r(string=db)ole_type`, which Dolt
// rejects with INVALID_ARGUMENT (gu-pwkh3 / escalation gc-wisp-8i9z).
...
func batchDeleteRows(ctx context.Context, db *sql.DB, idQuery, primaryTable string, auxTables []string, cutoffArgs ...interface{}) (int, error) {
	...
	for {
		...
		// Post-v49, parent-child links live in depends_on_wisp_id / depends_on_issue_id
		// (the legacy single-target column is empty), so target the column matching the
		// primary table to actually remove refs pointing at the purged rows. Non-fatal:
		// primary delete below is what matters.
