...
func TestReaperQueriesUseTypedDependencyColumns(t *testing.T) {
	...
	// The auto-close stale-issue WHERE clause is built by the autoCloseWhereClause
	// helper that sits immediately before AutoClose, so span from the helper to keep
...
func TestAutoAddCommitsAreGuarded(t *testing.T) {
	for _, sourcePath := range []string{"reaper.go", "hooked_mail.go"} {
		...
		for i, line := range lines {
			...
			// The guard runs on whichever SQL runner holds the working set — *sql.DB
			// for most sweeps, the pinned *sql.Conn in Reap's autocommit transaction.
			if !strings.Contains(window, "hasWorkingSetChanges(ctx, db)") && !strings.Contains(window, "hasWorkingSetChanges(ctx, conn)") {
...
func (c *fakeReaperConn) QueryContext(_ context.Context, query string, args []driver.NamedValue) (driver.Rows, error) {
	...
	case strings.Contains(normalized, "SELECT COUNT(*) FROM wisps w") && strings.Contains(normalized, "w.closed_at <"):
