...
type ReviseReplyArgs struct {
	...
	// Actor is the operator/agent who triggered the revision. Used in
	// the manual-dispatch template.
	Actor string
}
...
var ErrNoUnresolvedThread = errors.New("no unresolved comment thread found on MR bead")
...
//
// When ManualDispatch is true, the banner includes a "Dispatched by"
// line and uses the "Manual Revision" header.
...
//
// If commentID is non-empty, it is used directly (targeted path) — even
// when threads is empty or nil, the explicit ID is trusted.
//
// Otherwise (commentID is empty), the most recent non-resolved thread
// from threads is selected (D19 fallback path). Threads with an
// unparseable CreatedAt are skipped. Returns ErrNoUnresolvedThread when
// no eligible thread is found.
func ResolveTargetThread(commentID string, threads []CommentThread) (string, error) {
	...
	for i := range threads {
		...
		if err != nil {
			// If we can't parse the time, skip this thread — picking
			// an entry with garbage metadata is worse than failing.
			continue
		}
...
// the formula should reply on. When CommentID is set, exactly one reply
// targets that specific thread. When CommentID is empty, the fallback
// picks the most recent non-resolved thread and emits a single reply
// for it.
//
