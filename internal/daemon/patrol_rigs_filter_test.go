...
// Regression test for gs-3me:
// getPatrolRigs("main_branch_test") should filter parked/docked rigs.
func TestGetPatrolRigs_MainBranchTest_FiltersNonOperationalRigs(t *testing.T) {
	townRoot := t.TempDir()
	...
	slices.Sort(got)
	// Fail-safe: Dolt unavailable → isRigOperational returns false for all rigs.
	want := []string{}
	if !slices.Equal(got, want) {
		t.Fatalf("getPatrolRigs(main_branch_test) = %v, want %v (all rigs excluded when Dolt unavailable)", got, want)
	}
}
