...
func (dm *dogMol) close() {
	...
	// Retry the root close: a transient Dolt error must not leak the root wisp
	// (gt-ye21). The root-only wisp model means there are no child step wisps to
	// clean up — closing the root is sufficient.
