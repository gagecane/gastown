...
// This file contains the daemon heartbeat loop and its interval accessor.
// The heartbeat orchestrates every periodic check the daemon performs:
// agent liveness, patrol dispatch, GUPP enforcement, reaping, and scheduler
// dispatch. Individual step implementations live in topic-specific files
// (agents.go, polecat_health.go, reaper.go, etc.).
