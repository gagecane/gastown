...
// This file contains per-role "ensure running" helpers for all long-lived
// agents the daemon supervises: Boot, Deacon, Witnesses, Refineries, and
// Mayor. It also holds the kill-leftover-session helpers used when a patrol
// is disabled in daemon.json, and the default-prefix ghost cleaner. The
// companion file polecat_health.go handles polecat sessions, which require
// crash detection instead of just "is the session up".
