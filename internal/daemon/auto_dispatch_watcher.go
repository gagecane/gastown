...
func formatRigScopedAutoDispatchBody(p *plugin.Plugin, rig, trigger string) string {
	...
	sb.WriteString("5. Sort ready tasks by priority (P1 > P2 > P3). Pre-filter beads where ANY of:\n")
	...
	sb.WriteString(fmt.Sprintf("6. Iterate the sorted list. For each candidate, run `gt sling <task-id> %s`.\n", rig))
	sb.WriteString("   - **On success**: stop iteration; record `result:dispatched` with the bead ID.\n")
	sb.WriteString("   - **On `gt sling` rejection** (e.g. `epics cannot be scheduled with explicit rig`,\n")
	sb.WriteString("     `bead not found`, `not a known rig`, `wrong-rig:` label, etc.): log the\n")
	sb.WriteString("     rejection reason and CONTINUE to the next-highest-priority candidate.\n")
	sb.WriteString("     Do NOT abort the run on a single bead's rejection — the pre-filter is\n")
	sb.WriteString("     best-effort and the rejection is informative, not terminal (gu-060l).\n")
	sb.WriteString("7. Stop after the first successful dispatch OR when the candidate list is exhausted.\n")
	sb.WriteString("   - If no candidate slings successfully, record `result:no_dispatchable` with\n")
	sb.WriteString("     a brief summary (e.g. `tried 4 beads: 2 epics, 1 missing, 1 wrong-rig`).\n\n")
