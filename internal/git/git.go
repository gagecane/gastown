...
// slow gates (build/vet/test). The hook (scripts/pre-push-check.sh) requires
// GT_SKIP_PREPUSH_REASON alongside GT_SKIP_PREPUSH=1 so every skip lands in
// .runtime/prepush-skips.jsonl with a why — see gu-zy57. Callers that go
// through Push*SkipPrePush are always the --pre-verified path, hence the
// fixed reason; emergency / cherry-pick callers should use PushWithEnv and
// supply their own GT_SKIP_PREPUSH_REASON.
