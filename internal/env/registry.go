...
// This file is the single source of truth for every recognized GT_*
...
const (
	...
	GTContextBudgetMaxTokens Var = "GT_CONTEXT_BUDGET_MAX_TOKENS" //nolint:gosec // G101: token-budget var name, not a credential
	GTContextBudgetSoftGate  Var = "GT_CONTEXT_BUDGET_SOFT_GATE"
	GTContextBudgetTokens    Var = "GT_CONTEXT_BUDGET_TOKENS" //nolint:gosec // G101: token-budget var name, not a credential
...
// allSpecs returns every recognized GT_* var. Split out from init() so
...
func allSpecs() []Spec {
	return []Spec{
		...
		{GTFakeKiroMode, KindString, "", "Test-only: behavior selector for the fake-kiro stub."},
