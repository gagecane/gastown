...
case "$pos0" in
  list)
    ...
    ;;
  sql)
    # bdDepListRawIDs: SELECT COALESCE(...) AS target FROM dependencies WHERE issue_id = '<id>' AND type = 'tracks'
    case "$*" in
      *"issue_id = 'hq-empty-mix'"*)
        echo '[]'
        ;;
      *"issue_id = 'hq-feed-mix'"*)
        echo '[{"target":"gt-ready1"}]'
        ;;
      *)
        echo '[]'
        ;;
...
case "$pos0" in
  ...
  sql)
    ...
    echo '[{"target":"gt-busy1"},{"target":"gt-busy2"}]'
    exit 0
    ;;
  dep)
...
case "$pos0" in
  ...
  sql)
    ...
    case "$*" in
      *"IN ("*)
        echo '[{"issue_id":"hq-done1","target":"gt-closed1"},{"issue_id":"hq-done1","target":"gt-closed2"}]'
        ;;
      *)
        echo '[{"target":"gt-closed1"},{"target":"gt-closed2"}]'
        ;;
    esac
    exit 0
    ;;
