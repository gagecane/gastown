...
func TestParseContextInt(t *testing.T) {
	tests := []struct {
		in   string
		want int
	}{
	...
	for _, tt := range tests {
		got := parseContextInt(tt.in)
		if got != tt.want {
			t.Errorf("parseContextInt(%q) = %d, want %d", tt.in, got, tt.want)
		}
	}
}

func TestParseContextFloat(t *testing.T) {
	tests := []struct {
		in   string
		want float64
	}{
	...
	for _, tt := range tests {
		got := parseContextFloat(tt.in)
		if got != tt.want {
			t.Errorf("parseContextFloat(%q) = %f, want %f", tt.in, got, tt.want)
		}
	}
}

func TestResolveContextRole(t *testing.T) {
	// Save and restore env
	orig := os.Getenv("GT_ROLE")
	defer os.Setenv("GT_ROLE", orig)

	os.Setenv("GT_ROLE", "polecat/chrome")
	if got := resolveContextRole(); got != "polecat/chrome" {
		t.Errorf("resolveContextRole() = %q, want %q", got, "polecat/chrome")
	}
	...
	if got := resolveContextRole(); got != "polecat" {
		t.Errorf("resolveContextRole() with GT_POLECAT = %q, want %q", got, "polecat")
	}
}
