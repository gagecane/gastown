...
func (d *testDAG) BdStubScript() string {
	...
	// bdDepListRawIDs calls: bd sql "SELECT depends_on_issue_id FROM dependencies WHERE issue_id = '<id>' AND type = 'tracks'" --json
	// or: bd sql "SELECT issue_id FROM dependencies WHERE depends_on_issue_id = '<id>' AND type = 'tracks'" --json
	sb.WriteString("  sql\\ *)\n")
	...
	// For "up" direction (bead → tracking convoys): match on depends_on_issue_id = '<beadID>'
...
// trackedBeadsSQLJSONFor returns the JSON array for `bd sql "SELECT depends_on_issue_id
// FROM dependencies WHERE issue_id = '<convoyID>' AND type = 'tracks'" --json`.
// Returns [{"depends_on_issue_id":"<id>"},...] for each tracked bead.
...
// FROM dependencies WHERE depends_on_issue_id = '<beadID>' AND type = 'tracks'" --json`.
