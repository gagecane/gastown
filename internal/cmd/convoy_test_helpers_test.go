...
func (d *testDAG) BdStubScript() string {
	...
	// bdDepListRawIDs calls: bd sql "SELECT COALESCE(...) AS target FROM dependencies WHERE issue_id = '<id>' AND type = 'tracks'" --json
	// or: bd sql "SELECT issue_id FROM dependencies WHERE COALESCE(...) = '<id>' AND type = 'tracks'" --json
	sb.WriteString("  sql\\ *)\n")
	...
	// For "up" direction (bead → tracking convoys): the WHERE clause is now
	// COALESCE(...) = '<beadID>', so match on the COALESCE close + bead ID.
	...
	sb.WriteString("    exit 0\n")
	sb.WriteString("    ;;\n")
	sb.WriteString("  list\\ --json\\ --limit=0|list\\ --json\\ --limit=0\\ --all|list\\ --json\\ --limit=0\\ --all\\ --flat|list\\ --json\\ --limit=0\\ --status=*|list\\ --json\\ --limit=0\\ --flat*)\n")
	sb.WriteString("    echo '[]'\n")
	sb.WriteString("    exit 0\n")
	sb.WriteString("    ;;\n")
...
// trackedBeadsSQLJSONFor returns the JSON array for `bd sql "SELECT COALESCE(...)
// AS target FROM dependencies WHERE issue_id = '<convoyID>' AND type = 'tracks'" --json`.
// Returns [{"target":"<id>"},...] for each tracked bead.
func (d *testDAG) trackedBeadsSQLJSONFor(convoyID string) string {
	type sqlRow struct {
		DependsOnID string `json:"target"`
	}
...
// FROM dependencies WHERE COALESCE(...) = '<beadID>' AND type = 'tracks'" --json`.
