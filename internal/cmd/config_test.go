...
func TestConfigSetGet(t *testing.T) {
	...
	t.Run("set and get scheduler.max_load_per_core", func(t *testing.T) {
		...
		if got := loaded.Scheduler.GetMaxLoadPerCore(); got != 2.5 {
			t.Errorf("max_load_per_core = %v, want 2.5", got)
		}
	...
	t.Run("scheduler.max_load_per_core rejects negative", func(t *testing.T) {
		townRoot := setupTestTownForConfig(t)
