...
func runPolecatNuke(cmd *cobra.Command, args []string) error {
	...
	for _, p := range targets {
		...
		if err := nukePolecatFullWithOptions(p.polecatName, p.rigName, p.mgr, p.r, polecatNukeForce, nukePolecatOptions{PurgeClosedEphemerals: !batchPurge}); err != nil {
	...
	if polecatStaleCleanup && staleCount > 0 {
		...
		if polecatStaleDryRun {
			...
			for _, info := range staleInfos {
				...
				fmt.Printf("Nuking %s...\n", info.Name)
				if err := nukePolecatFullWithOptions(info.Name, rigName, mgr, r, false, nukePolecatOptions{PurgeClosedEphemerals: !batchPurge}); err != nil {
