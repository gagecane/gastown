...
// The convoy staging DAG state machine lives in internal/convoy
// (staging.go). These aliases and delegators keep the cmd-layer call sites and
// the existing test suite working against the canonical types. See gu-nid89.12.5.

// ConvoyDAG is an alias for the canonical convoy.ConvoyDAG.
type ConvoyDAG = convoy.ConvoyDAG

// ConvoyDAGNode is an alias for the canonical convoy.ConvoyDAGNode.
type ConvoyDAGNode = convoy.ConvoyDAGNode

// Wave is an alias for the canonical convoy.Wave.
type Wave = convoy.Wave
...
// computeWaves delegates to convoy.ComputeWaves (Kahn's-algorithm wave
// scheduler). Kept as a cmd-layer alias for command wiring and tests.
func computeWaves(dag *ConvoyDAG) ([]Wave, []GatedTask, error) {
	return convoy.ComputeWaves(dag)
}
...
// BeadInfo is an alias for the canonical convoy.BeadInfo.
type BeadInfo = convoy.BeadInfo

// DepInfo is an alias for the canonical convoy.DepInfo.
type DepInfo = convoy.DepInfo

// StagingFinding is an alias for the canonical convoy.StagingFinding.
type StagingFinding = convoy.StagingFinding

// buildConvoyDAG delegates to convoy.BuildConvoyDAG.
func buildConvoyDAG(beads []BeadInfo, deps []DepInfo) *ConvoyDAG {
	return convoy.BuildConvoyDAG(beads, deps)
}

// categorizeFindings delegates to convoy.CategorizeFindings.
func categorizeFindings(findings []StagingFinding) (errors, warnings []StagingFinding) {
	return convoy.CategorizeFindings(findings)
}

// detectErrors delegates to convoy.DetectErrors.
func detectErrors(dag *ConvoyDAG) []StagingFinding { return convoy.DetectErrors(dag) }

// chooseStatus delegates to convoy.ChooseStatus.
func chooseStatus(errors, warnings []StagingFinding) string {
	return convoy.ChooseStatus(errors, warnings)
}
