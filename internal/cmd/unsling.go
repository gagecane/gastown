...
Related commands:
  ...
  gt hook      # See what's on your hook`,
	...
	// Without this, any RunE error makes cobra dump the full usage/help text,
	// which reads like a syntax mistake. unsling errors are operational (bead
	// not on the active hook slot, work incomplete, etc.), not arg-parse
	// failures, so the help dump is misleading (gu-u7num).
	SilenceUsage: true,
	RunE:         runUnsling,
}
