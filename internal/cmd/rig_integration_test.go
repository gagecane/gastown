...
func checkWorktreeClean(t *testing.T, agent agentWorktree, hasTrackedBeads bool) []string {
	...
	if hasTrackedBeads {
		...
		allowlist["M .beads/config.yaml"] = true           // EnsureConfigYAML adds Gas Town defaults (idle-timeout, auto-commit, export.auto) when source has minimal config — gu-8nbc / gu-1kaw
