...
# Test 14: Off-branch push — HEAD on a session branch, pushing the default branch
# (the classic `git push origin main` from a feature branch). HEAD mismatch — BLOCKED.
echo "Test 14: Off-branch push (HEAD on session branch, pushing default)"
setup_repos
cd "$TMPDIR/local"
...
# Test 15: Off-branch push with GT_ALLOW_OFFBRANCH_PUSH=1 — ALLOWED (override).
echo "Test 15: Off-branch push with GT_ALLOW_OFFBRANCH_PUSH=1"
setup_repos
cd "$TMPDIR/local"
...
# Test 16: Off-branch deletion push (zero local sha) — not a HEAD mismatch, ALLOWED.
echo "Test 16: Off-branch deletion push (zero local sha)"
setup_repos
cd "$TMPDIR/local"
