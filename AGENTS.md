...
<!-- Hand-maintained guidance below this line is NOT regenerated by bd/gt tooling. -->

## Git: refresh `origin/main` before diffing or reviewing (footgun)

In a rig worktree, **`git fetch origin main` does NOT reliably update the
`origin/main` tracking ref.** It always writes `FETCH_HEAD`, but updates
`refs/remotes/origin/main` only *opportunistically* — and not at all when the
shared bare repo's `remote.origin.fetch` refspec is missing or narrow. A stale
`origin/main` silently makes `git diff origin/main...` and
`git log origin/main..HEAD` report against an old base. This has posted **false
PR-review findings to a customer PR** (origin/main sat ~14 commits behind for an
entire session).

Before any diff / log / base claim against `origin/main`, refresh the tracking
ref explicitly so it cannot go stale:

```bash
git fetch origin                              # updates refs/remotes/origin/* via the configured refspec
# or force just the one ref, independent of refspec config:
git fetch origin main:refs/remotes/origin/main
```

Then verify the tracking ref actually advanced before trusting a diff:

```bash
git rev-parse origin/main FETCH_HEAD          # both SHAs should match after the fetch
```

If `origin/main` keeps going stale even after `git fetch origin`, the bare repo's
refspec is wrong (the root cause). Repair it with:

```bash
gt doctor --fix --rig <rig>                   # fixes remote.origin.fetch on .repo.git
```
