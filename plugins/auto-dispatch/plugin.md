...
2. For each rig:
   ...
   b. Filter out non-dispatchable beads and any that have unmet blockers. A bead is non-dispatchable if ANY of:
      ...
      - The bead `owner` address identifies a Gas Town agent rather than a human (canonical forms: `mayor`, `mayor/`, `<rig>/witness`, `<rig>/refinery`, `<rig>/crew/<name>`, `<rig>/polecats/<name>`, `deacon`, `deacon/dogs/<name>`). Agent-owned beads are orchestrator state or coordination work that the owning agent handles directly — slinging them to a polecat in the bead's rig is a target mismatch and either fails (`'mayor' is not a known rig`) or wastes a polecat slot on work that does not belong to it (gs-myq / 2026-05-19 lb-wfs-ef6cs incident: workflow-orchestrator bead with `workflow_target: mayor` slung as if mayor were a rig). A simple heuristic: if the owner string contains a `/` or matches a known agent role (`mayor`, `deacon`), treat as agent-owned and skip. **Exception:** workflow step beads (id matches `*-wfs-*`) and convoy synthesis beads (id matches `*-syn-*`) are real polecat work, not orchestrator state — `gt formula run` and convoy pour (`gt sling <convoy>`) stamp them with the owner of whoever launched the run, so a crew-launched workflow/convoy yields step beads owned by `<rig>/crew/<name>`. Without an exception the agent-owner filter silently drops them from the fast dispatch path. For `*-wfs-*` steps this left them to advance only on the Deacon's slow stranded-feed cycle (gu-3y6ro); for `*-syn-*` synthesis it is worse — legs are slung at pour time but the synthesis bead is only created (blocked on the legs) and never slung, so the auto-dispatch bd-ready scan is the ONLY producer of its sling context. Skipping it as agent-owned strands the synthesis bead indefinitely until a human manually slings it (gs-guid). Steps that genuinely route to a specific agent are still caught by the `workflow_target:` filter, and `gt sling`'s server-side guards remain the backstop.
      ...
      - When dispatching to rig `<rig>`, the bead carries a `wrong-rig:<rig>` label asserting that prior dispatch to that rig was wrong (gu-mhfs). This breaks the cross-rig mis-routing loop where the failure-classifier persistently picks the same wrong rig for a test exercising code in another rig (cala-7e9 / cala-tl5 → obsidian on casc_lambda for auth-enforcement, despite the auth wiring living in casc_cdk and the HTTP handler in casc_crud). Multiple labels stack: a bead wrong in two rigs carries two labels and only the third (or human triage) sees it. Polecats should add the label when closing no-changes for owning-package mismatches; operators can also apply it manually.
      - The bead is a CDK/Python (or other per-package) **code task whose code target resolves to a path the target rig gitignores** — most commonly a MetaRepo (`ti-*`) bead whose work lands under `workspaces/`. Such beads are structurally un-landable in a MetaRepo rig: the rig gitignores `workspaces/` and its `pre-merge-check.sh` fails any MR that tracks files under `workspaces/` (besides `.gitkeep`), so the polecat's commits can never merge and it loops on re-dispatch (root cause of the ticd-a3b incident; gu-na4id). Do NOT sling such a bead to the MetaRepo rig and do NOT author a code task as a `ti-*` MetaRepo bead in the first place — route it to the correct per-package merge rig (`ticd-*` / `tite-*`) whose checkout tracks that package's source instead. If no per-package rig exists for the target, leave the bead for human/mayor triage rather than dispatching it into a gitignored path.
