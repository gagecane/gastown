...
# rig once per day (cron gate: 0 9 * * *), letting `gt sling` auto-resolve a polecat.
#
...
# daemon level (DispatchGrace suppresses re-fire). The single-instance check
# prevents double-dispatch if the gate is bypassed (e.g. --force). gt sling
# itself is idempotent for already-hooked beads, but we never want a *new*
# wisp-pair while an earlier one is still in flight.
